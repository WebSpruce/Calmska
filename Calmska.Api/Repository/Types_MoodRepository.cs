...
namespace Calmska.Api.Repository
{
    public class Types_MoodRepository : ITypesRepository<Types_Mood, Types_MoodDTO>
    {
        ...
        public async Task<PaginatedResult<Types_Mood>> GetAllAsync(int? pageNumber, int? pageSize)
        {
            var query = await Task.Run(_context.Types_MoodDb.AsQueryable);
        ...
        public async Task<Types_Mood?> GetByArgumentAsync(Types_MoodDTO moods)
        {
        ...
        public async Task<OperationResult> AddAsync(Types_MoodDTO Types_MoodDTO)
        {
            try
            {
                ...
                await _context.Types_MoodDb.AddAsync(typemood);
        ...
        public async Task<OperationResult> UpdateAsync(Types_MoodDTO moods)
        {
            try
            {
                if (moods == null)
                ...
                Types_Mood? existingTypemood = await _context.Types_MoodDb.FirstOrDefaultAsync(t => t.TypeId == moods.TypeId);
        ...
        public async Task<OperationResult> DeleteAsync(int type_moodId)
        {
            try
            {
                if (type_moodId <= 0)
                ...
                var typeObject = await _context.Types_MoodDb.FirstOrDefaultAsync(t => t.TypeId == type_moodId);
                if (typeObject == null)
